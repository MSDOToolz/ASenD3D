...
impl Element {
    ...
    pub fn get_ruk_dfd0(&mut self, rvec : &mut Vec<DiffDoub0>, d_rdu : &mut Vec<f64>, d_rd_t : &mut Vec<f64>, d_rd_c : &mut Vec<f64>, get_matrix : bool, n_lgeom : bool, pre : &mut DiffDoub0StressPrereq) {
        let mut i3 : usize;
        let mut i4 : usize;
        let mut i5 : usize;
        let mut i6 : usize;
        ...
        let mut diff_stn = [DiffDoub0::new(); 6];
        ...
        let mut tmp_de = [DiffDoub0::new(); 6];
        ...
        for i1 in 0..self.num_ip {
            ...
            for i2 in 0..self.num_nds {
                ...
                tmp.mult(& n_vec[i2]);
                ip_temp.add(& tmp);
                tmp.set_val_dfd0(&pre.glob_fl_den[i2]);
                tmp.mult(&n_vec[i2]);
                ip_con.add(&tmp);
            }
            if self.dof_per_nd == 3 {
                ...
                for i2 in 0..6 {
                    ...
                    diff_stn[i2].set_val_dfd0(& pre.diff_exp[i2]);
                    diff_stn[i2].mult(&ip_con);
                    strain[i2].sub(& thrm_stn[i2]);
                    strain[i2].sub(&diff_stn[i2]);
                ...
                for i2 in 0..6 {
                    ...
                    tmp.set_val_dfd0(&pre.diff_exp[i2]);
                    tmp.mult(&ip_con);
                    sec_fc_mom[i2].sub(&tmp);
    ...
    pub fn get_rud_dfd0(&mut self, rvec : &mut Vec<DiffDoub0>, d_rd_v : &mut Vec<f64>, get_matrix : bool, cmd : & JobCommand, pre : &mut DiffDoub0StressPrereq, 
        ...
        if cmd.ray_damp_ck > 0.0 {
            ...
            self.get_ruk_dfd0(rtmp,  d_rtmp,  d_rd_t, d_rd_c, get_matrix, cmd.nonlinear_geom, pre);
            for i1 in 0..nd_dof {
                rvec[i1].add(& rtmp[i1]);
            }
            if get_matrix {
    ...
    pub fn get_ru_dfd0(&mut self, glob_r : &mut Vec<DiffDoub0>, globd_rdu : &mut SparseMat, get_matrix : bool, cmd : & JobCommand, pre : &mut DiffDoub0StressPrereq, 
        ...
        if cmd.dynamic {
            if get_matrix {
                ...
                if self.this_type != 1 {
                    if cmd.explicit {
                        self.get_rud_dfd0(rtmp, d_rtmp, false, cmd, pre, scr, scr_dfd);
                        for i1 in 0..nd_dof {
                            rvec[i1].add(&rtmp[i1]);
                        }
                        // i2 = 0;
        ...
        for i1 in 0..nd_dof {
            ...
            if get_matrix {
                if cmd.explicit {
                    i3 = i1*(tot_dof + 1);
                    globd_rdu.add_entry(glob_ind, glob_ind, d_rdu[i3]);
                }
                else {
                    i3 = i1*tot_dof;
                    i5 = 0;
    ...
    pub fn get_rt_dfd0(&mut self, glob_r : &mut Vec<DiffDoub0>, globd_rd_t : &mut SparseMat, get_matrix : bool, cmd : & JobCommand, pre : &mut DiffDoub0StressPrereq, 
        ...
        let mut glob_ind2 : usize;
        let mut temp_t = [DiffDoub0::new(); 10];
        //let c1 : f64 =  1.0/(cmd.time_step*cmd.newmark_gamma);
        let c1 : f64 = match cmd.explicit {
            true => 0.5 / cmd.time_step,
            false => 1.0 / (cmd.time_step*cmd.newmark_gamma),
        };
        //let mut rvec = &mut scr.scr_v1;
        ...
        if self.this_type != 1 {
            self.get_rtk_dfd0(&mut rvec, &mut  d_rd_t,  get_matrix && !cmd.explicit,  pre);
        }
    ...
    pub fn get_rdk_dfd0(&mut self, rvec : &mut Vec<DiffDoub0>, d_rd_rho : &mut Vec<f64>, get_matrix : bool, pre : &mut DiffDoub0StressPrereq) {
        let mut temp_copy = [DiffDoub0::new(); 10];
        let mut cond_copy = [DiffDoub0::new(); 9];
        
        for i in 0..10 {
            temp_copy[i].set_val_dfd0(&pre.glob_temp[i]);
            pre.glob_temp[i].set_val_dfd0(&pre.glob_fl_den[i]);
        }
        for i in 0..9 {
            cond_copy[i].set_val_dfd0(&pre.tcmat[i]);
            pre.tcmat[i].set_val_dfd0(&pre.diff_mat[i]);
        }
        
        self.get_rtk_dfd0(rvec, d_rd_rho, get_matrix, pre);
        
        for i in 0..10 {
            pre.glob_temp[i].set_val_dfd0(&temp_copy[i]);
        }
        for i in 0..9 {
            pre.tcmat[i].set_val_dfd0(&cond_copy[i]);
        }
    }

    pub fn get_rdm_dfd0(& self, rvec : &mut Vec<DiffDoub0>, d_rd_rhodot : &mut Vec<f64>, get_matrix : bool, actual_props : bool, pre : &mut DiffDoub0StressPrereq) {
        let mut sh_copy = DiffDoub0::new();
        let mut m_copy = DiffDoub0::new();
        let mut td_copy = [DiffDoub0::new(); 10];

        sh_copy.set_val_dfd0(&pre.spec_heat);
        pre.spec_heat.set_val(1.0);
        m_copy.set_val_dfd0(&pre.mmat[0]);
        pre.mmat[0].set_val(1.0);
        for i in 0..10 {
            td_copy[i].set_val_dfd0(&pre.glob_tdot[i]);
            pre.glob_tdot[i].set_val_dfd0(&pre.glob_fl_den_dot[i]);
        }

        self.get_rtm_dfd0(rvec, d_rd_rhodot, get_matrix, actual_props, pre);

        pre.spec_heat.set_val_dfd0(&sh_copy);
        pre.mmat[0].set_val_dfd0(&m_copy);
        for i in 0..10 {
            pre.glob_tdot[i].set_val_dfd0(&td_copy[i]);
        }
    }

    pub fn get_rd_dfd0(&mut self, glob_r : &mut Vec<DiffDoub0>, globd_rd_rho : &mut SparseMat, get_matrix : bool, cmd : & JobCommand, pre : &mut DiffDoub0StressPrereq,
        scr : &mut IterMut<'_,FltScr>, scr_dfd : &mut IterMut<'_,DiffDoub0Scr>, nd_ar : &mut Vec<Node>) {
        ...
        self.get_rt_dfd0(glob_r, globd_rd_rho, get_matrix, cmd, pre, scr, scr_dfd, nd_ar);
    ...
    pub fn get_app_diff_load_dfd0(&self, app_ld : &mut Vec<DiffDoub0>, ld_pt : &mut Load, time : f64, pre : &mut DiffDoub0StressPrereq, 
        ...
        if !"massGen massFlux".contains(&type_copy) {
            println!("Warning: Unrecognized load type for diffusion: '{}'. Ignoring load entry.", type_copy);
            return;
        }

        ld_pt.this_type = match type_copy.as_str() {
            "massGen" => CppStr::from("bodyHeatGen"),
            "massFlux" => CppStr::from("surfaceFlux"),
            &_ => CppStr::from("none"),
        };
...
//DiffDoub1 versions: 
    ...
    pub fn get_ruk_dfd1(&mut self, rvec : &mut Vec<DiffDoub1>, d_rdu : &mut Vec<f64>, d_rd_t : &mut Vec<f64>, d_rd_c : &mut Vec<f64>, get_matrix : bool, n_lgeom : bool, pre : &mut DiffDoub1StressPrereq) {
        let mut i3 : usize;
        let mut i4 : usize;
        let mut i5 : usize;
        let mut i6 : usize;
        ...
        let mut diff_stn = [DiffDoub1::new(); 6];
        ...
        let mut tmp_de = [DiffDoub1::new(); 6];
        ...
        for i1 in 0..self.num_ip {
            ...
            for i2 in 0..self.num_nds {
                ...
                tmp.mult(& n_vec[i2]);
                ip_temp.add(& tmp);
                tmp.set_val_dfd1(&pre.glob_fl_den[i2]);
                tmp.mult(&n_vec[i2]);
                ip_con.add(&tmp);
            }
            if self.dof_per_nd == 3 {
                ...
                for i2 in 0..6 {
                    ...
                    thrm_stn[i2].mult(& ip_temp);
                    diff_stn[i2].set_val_dfd1(& pre.diff_exp[i2]);
                    diff_stn[i2].mult(&ip_con);
                    strain[i2].sub(& thrm_stn[i2]);
                    strain[i2].sub(&diff_stn[i2]);
                    strain[i2].sub(& pre.einit[i2]);
                ...
                for i2 in 0..6 {
                    ...
                    tmp.mult(& ip_temp);
                    sec_fc_mom[i2].sub(&tmp);
                    tmp.set_val_dfd1(&pre.diff_exp[i2]);
                    tmp.mult(&ip_con);
                    sec_fc_mom[i2].sub(&tmp);
                    sec_fc_mom[i2].sub(&pre.einit[i2]);
                }
    ...
    pub fn get_rud_dfd1(&mut self, rvec : &mut Vec<DiffDoub1>, d_rd_v : &mut Vec<f64>, get_matrix : bool, cmd : & JobCommand, pre : &mut DiffDoub1StressPrereq, 
        ...
        if cmd.ray_damp_ck > 0.0 {
            ...
            self.get_ruk_dfd1(rtmp,  d_rtmp,  d_rd_t, d_rd_c, get_matrix, cmd.nonlinear_geom, pre);
            for i1 in 0..nd_dof {
                rvec[i1].add(& rtmp[i1]);
            }
            if get_matrix {
    ...
    pub fn get_ru_dfd1(&mut self, glob_r : &mut Vec<DiffDoub1>, globd_rdu : &mut SparseMat, get_matrix : bool, cmd : & JobCommand, pre : &mut DiffDoub1StressPrereq, 
        ...
        if cmd.dynamic {
            if get_matrix {
                ...
                if self.this_type != 1 {
                    if cmd.explicit {
                        self.get_rud_dfd1(rtmp, d_rtmp, false, cmd, pre, scr, scr_dfd);
                        for i1 in 0..nd_dof {
                            rvec[i1].add(&rtmp[i1]);
                        }
                        // i2 = 0;
    ...
    pub fn get_rt_dfd1(&mut self, glob_r : &mut Vec<DiffDoub1>, globd_rd_t : &mut SparseMat, get_matrix : bool, cmd : & JobCommand, pre : &mut DiffDoub1StressPrereq, 
        ...
        let mut glob_ind1 : usize;
        let mut glob_ind2 : usize;
        let mut temp_t = [DiffDoub1::new(); 10];
        //let c1 : f64 =  1.0/(cmd.time_step*cmd.newmark_gamma);
        let c1 : f64 = match cmd.explicit {
            true => 0.5 / cmd.time_step,
            false => 1.0 / (cmd.time_step*cmd.newmark_gamma),
        ...
        if self.this_type != 1 {
            self.get_rtk_dfd1(&mut rvec, &mut  d_rd_t,  get_matrix && !cmd.explicit,  pre);
        }
    ...
    pub fn get_rdk_dfd1(&mut self, rvec : &mut Vec<DiffDoub1>, d_rd_rho : &mut Vec<f64>, get_matrix : bool, pre : &mut DiffDoub1StressPrereq) {
        let mut temp_copy = [DiffDoub1::new(); 10];
        let mut cond_copy = [DiffDoub1::new(); 9];
        
        for i in 0..10 {
            temp_copy[i].set_val_dfd1(&pre.glob_temp[i]);
            pre.glob_temp[i].set_val_dfd1(&pre.glob_fl_den[i]);
        }
        for i in 0..9 {
            cond_copy[i].set_val_dfd1(&pre.tcmat[i]);
            pre.tcmat[i].set_val_dfd1(&pre.diff_mat[i]);
        }
        
        self.get_rtk_dfd1(rvec, d_rd_rho, get_matrix, pre);
        
        for i in 0..10 {
            pre.glob_temp[i].set_val_dfd1(&temp_copy[i]);
        }
        for i in 0..9 {
            pre.tcmat[i].set_val_dfd1(&cond_copy[i]);
        }
    }

    pub fn get_rdm_dfd1(& self, rvec : &mut Vec<DiffDoub1>, d_rd_rhodot : &mut Vec<f64>, get_matrix : bool, actual_props : bool, pre : &mut DiffDoub1StressPrereq) {
        let mut sh_copy = DiffDoub1::new();
        let mut m_copy = DiffDoub1::new();
        let mut td_copy = [DiffDoub1::new(); 10];

        sh_copy.set_val_dfd1(&pre.spec_heat);
        pre.spec_heat.set_val(1.0);
        m_copy.set_val_dfd1(&pre.mmat[0]);
        pre.mmat[0].set_val(1.0);
        for i in 0..10 {
            td_copy[i].set_val_dfd1(&pre.glob_tdot[i]);
            pre.glob_tdot[i].set_val_dfd1(&pre.glob_fl_den_dot[i]);
        }

        self.get_rtm_dfd1(rvec, d_rd_rhodot, get_matrix, actual_props, pre);

        pre.spec_heat.set_val_dfd1(&sh_copy);
        pre.mmat[0].set_val_dfd1(&m_copy);
        for i in 0..10 {
            pre.glob_tdot[i].set_val_dfd1(&td_copy[i]);
        }
    }

    pub fn get_rd_dfd1(&mut self, glob_r : &mut Vec<DiffDoub1>, globd_rd_rho : &mut SparseMat, get_matrix : bool, cmd : & JobCommand, pre : &mut DiffDoub1StressPrereq,
        scr : &mut IterMut<'_,FltScr>, scr_dfd : &mut IterMut<'_,DiffDoub1Scr>, nd_ar : &mut Vec<Node>) {
        ...
        self.get_rt_dfd1(glob_r, globd_rd_rho, get_matrix, cmd, pre, scr, scr_dfd, nd_ar);
