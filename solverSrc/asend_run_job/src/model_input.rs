...
impl Model {
    ...
    pub fn read_model_input(&mut self, file_name : &mut CppStr) {
        ...
        let mut lay_ct : usize =  0;
        let mut set_ind : usize =  0;
            ...
            for line in lines.map_while(Result::ok) {
                ...
                if (headings[0].s == "nodes" && data_len == 4) {
                    ...
                    if(headings[1].s == "type" && data_len == 1) {
                        ...
                        let mut new_el = &mut self.elements[el_ct];
                    ...
                    if(headings[1].s == "node") {
                        if(headings[2].s == "name" && data_len == 1) {
                            ...
                            let mut new_set : &mut Set = &mut self.node_sets[ns_ct];
                        ...
                        if(headings[2].s == "name" && data_len == 1) {
                            ...
                            let mut new_set = &mut self.element_sets[es_ct];
                            new_set.name = data[0].clone();
                        } else if(headings[2].s == "labels") {
                            if(data_len == 1) {
                    ...
                    if(headings[1].s == "name" && data_len == 1) {
                        ...
                        if(headings[2].s == "E" && data_len > 0) {
                            let mut this_mat = &mut self.materials[mat_ct];
                        ...
                        if(headings[2].s == "maxStress") {
                            if(headings[3].s == "tensile" && data_len > 0) {
                                let mut n_mat = &mut self.materials[mat_ct];
        ...
        let mut labs : &mut LinkedList<usize> = &mut self.node_sets[ns_ct].labels;
        for i2 in 0..i1 {
            labs.push_back(i2);
        }
        ...
        es_ct += 1usize;
        let mut new_set : &mut Set = &mut self.element_sets[es_ct];
    ...
    pub fn read_constraint_input(&mut self, file_name : &mut CppStr) {
        ...
        let mut i1 : usize;
        let mut all_types = CppStr::from("displacement temperature");
        let mut err_st = CppStr::new();
        
        let mut ec_ct : usize =  0;
        let mut tc_ct : usize =  0;
        let mut this_type = CppStr::new();
        let mut curr_tm : usize;
