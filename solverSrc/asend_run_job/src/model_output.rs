...
impl Model {
    pub fn write_time_step_soln(&mut self, t_step : usize) {
        ...
        for nd in self.nodes.iter() {
            if (self.job[self.solve_cmd].thermal) {
                out_file.write(&nd.prev_temp.to_be_bytes());
                out_file.write(&nd.prev_tdot.to_be_bytes());
            }
            if (self.job[self.solve_cmd].elastic) {
                for i in 0..nd.num_dof {
                    out_file.write(&nd.prev_disp[i].to_be_bytes());
                }

                for i in 0..nd.num_dof {
                    out_file.write(&nd.prev_vel[i].to_be_bytes());
                }

                for i in 0..nd.num_dof {
                    out_file.write(&nd.prev_acc[i].to_be_bytes());
                }
            ...
            for el in self.elements.iter() {
                if (el.num_int_dof > 0) {
                    for i in 0..el.num_int_dof {
                        out_file.write(&el.int_prev_disp[i].to_be_bytes());
                    }
                }
            }
        }
    ...
    pub fn write_node_results(&mut self, file_name : &mut CppStr, node_set : &mut CppStr, fields : &mut LinkedList<CppStr>, time_step : usize) {
        ...
        let mut set_pt : usize;
        let mut err_str = CppStr::new();
        let mut this_field = CppStr::new();
        ...
        let mut out_file = match File::create(&file_name.s) {
            Err(why) => panic!("could not open file {}", file_name.s),
            Ok(file) => file,
        };
        
        out_file.write(format!("{}", "nodeResults:\n").as_bytes());
        out_file.write(format!("{}{}{}", "    time: " , time , "\n").as_bytes());
        out_file.write(format!("{}{}{}", "    nodeSet: " , node_set.s , "\n").as_bytes());
    ...
    pub fn write_element_results(&mut self, file_name : &mut CppStr, el_set : &mut CppStr, fields : &mut LinkedList<CppStr>, position : &mut CppStr, time_step : usize) {
        ...
        let mut cent : [f64; 3] = [ 0.0,0.0,0.0 ];
        ...
        let mut this_set : &Set = &self.element_sets[set_pt];
        ...
        out_file.write(format!("{}", "elementResults:\n").as_bytes());
        out_file.write(format!("{}{}{}", "    time: " , time , "\n").as_bytes());
        out_file.write(format!("{}{}{}", "    elSet: " , el_set.s , "\n").as_bytes());
        ...
        for this_field in fields.iter_mut() {
            ...
            if (this_field.s == "heatFlux") {
                out_file.write(format!("{}", "    ##  - [element label, integration pt, layer, f1, f2, f3]\n").as_bytes());
            }
            if (this_field.s == "tempGradient") {
                out_file.write(format!("{}", "    ##  - [element label, integration pt, layer, dT/dx, dT/dy, dT/dz]\n").as_bytes());
            }
    ...
    pub fn write_modal_results(&mut self, file_name : &mut CppStr, write_modes : bool) {
        let mut i1 : usize;
        let mut i2 : usize;
        let mut i3 : usize;
        let mut nd : usize;
        ...
        let mut glob_ind : usize;
        let mut mcmd = &self.job[self.modal_cmd];
        let mut n_mds : usize =  mcmd.num_modes;
        ...
        if (mcmd.write_modes) {
            out_file.write(format!("{}", "    modes:\n").as_bytes());
            for i1 in 0..n_mds {
                out_file.write(format!("{}{}{}", "      - mode: " , i1 , "\n").as_bytes());
                out_file.write(format!("{}", "        displacement:\n").as_bytes());
                for this_nd in self.nodes.iter_mut() {
                    nd = this_nd.label;
                    out_file.write(format!("{}{}", "          - [" , nd).as_bytes());
    ...
    pub fn write_objective(&mut self, file_name : &mut CppStr, include_fields : &mut LinkedList<CppStr>, write_grad : bool) {
        let mut i1 : usize;
        let mut num_dv : usize;
        let mut tot_obj : f64;
        let mut this_val : f64;
        let mut fld_str = CppStr::new();
        ...
        out_file.write(format!("{}", "objective:\n").as_bytes());
        out_file.write(format!("{}", "    terms:\n").as_bytes());
        tot_obj = 0.0;
        for this_term in self.obj.terms.iter_mut() {
            ...
            for fld_str in include_fields.iter_mut() {
                if (fld_str.s == "category") {
                    ...
                    out_file.write(format!("{}{}{}", "          operator: " , this_term.optr.s , "\n").as_bytes());
                }
                else if (fld_str.s == "component") {
                    out_file.write(format!("{}{}{}", "          component: " , this_term.component , "\n").as_bytes());
                }
