...
impl Constraint {
    ...
    pub fn build_mat(&mut self, nd_ar : &Vec<Node>, set_ar : &Vec<Set>) {
        ...
        for tm in self.terms.iter_mut() {
            ...
            if set_labs.len() == 1 {
                ...
                nd_sti = 6*nd_ar[nd_index].sorted_rank;
                col = match self.this_type.s.as_str() {
                    "density" => nd_sti,
                    "velocity" => nd_sti + dof,
                    "temperature" => nd_sti + 4,
                    "turbulence" => nd_sti + 5,
                    &_ => panic!("Error: unrecognized fluid constraint type, {}", self.this_type.s),
                };
    ...
    pub fn add_to_sparse_mat(&self, sp_mat : &mut SparseMat) {
        let mut col1 : usize;
        let mut col2 : usize;

        for cr in self.mat.matrix.iter() {
            for ent1 in cr.row_vec.iter() {
                col1 = ent1.col;
                for ent2 in cr.row_vec.iter() {
                    col2 = ent2.col;
                    sp_mat.add_entry(col1, col2, ent1.value*ent2.value*self.scale_fact);
                }
            }
        }
    }
