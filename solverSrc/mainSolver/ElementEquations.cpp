...
void Element::get_ruk_dfd0(vector<DiffDoub0>& rvec, vector<double>& d_rdu, vector<double>& d_rd_t, bool get_matrix, bool n_lgeom, DiffDoub0StressPrereq& pre, vector<Node>& nd_ar, vector<DesignVariable>& dv_ar) {
	int i1;
	int i2;
	int i3;
	int i4;
	...
	vec_to_ar_dfd0(tmp_c, pre.cmat, 0, 81);
	vec_to_ar_dfd0(tmp_gd, pre.glob_disp, 0, 60);
	vec_to_ar_dfd0(tmp_te, pre.therm_exp, 0, 6);
	...
	for (i1 = 0; i1 < num_ip; i1++) {
		get_ip_data_dfd0(n_vec,d_ndx,det_j,pre.loc_nds,&int_pts[3*i1]);
		...
		if(dof_per_nd == 3) {
			mat_mul_ar_dfd0(ux,tmp_gd,d_ndx,3,n_dim,3);
			...
			mat_mul_ar_dfd0(stress,tmp_c,strain,6,6,1);
			if (get_matrix) {
				mat_mul_ar_dfd0(cte, tmp_c, tmp_te, 6, 6, 1);
				mat_mul_ar_dfd0(cten, cte, n_vec, 6, 1, num_nds);
			}
			for (i2 = 0; i2 < tot_dof; i2++) {
				get_solid_strain_dfd0(strain,ux,d_ndx,pre.loc_ori,i2,max_int,n_lgeom);
				i4 = i2;
				for (i3 = 0; i3 < 6; i3++) {
				...
				if(n_lgeom && get_matrix) {
					...
					for (i3 = i2; i3 < tot_dof; i3++) {
						get_solid_strain_dfd0(strain,ux,d_ndx,pre.loc_ori,i2,i3,n_lgeom);
						for (i4 = 0; i4 < 6; i4++) {
			...
			mat_mul_ar_dfd0(sec_fc_mom,tmp_c,sec_def,def_dim,def_dim,1);
			for (i2 = 0; i2 < 6; i2++) {
			...
			if (get_matrix) {
				mat_mul_ar_dfd0(cten, tmp_te, n_vec, 6, 1, num_nds);
			}
			for (i2 = 0; i2 < tot_dof; i2++) {
				get_section_def_dfd0(sec_def,pre.glob_disp,pre.inst_ori,pre.loc_ori,pre.glob_nds,d_ndx,n_vec,n_lgeom,i2,max_int);
				i4 = i2;
				...
				if(n_lgeom && get_matrix) {
					...
					i6 = i5;
					for (i3 = i2; i3 < tot_dof; i3++) {
						get_section_def_dfd0(sec_def,pre.glob_disp,pre.inst_ori,pre.loc_ori,pre.glob_nds,d_ndx,n_vec,n_lgeom,i2,i3);
		...
		if(get_matrix) {
			mat_mul_dfd0(pre.cbmat,pre.cmat,pre.bmat,def_dim,def_dim,tot_dof);
			i3 = def_dim*tot_dof;
			for (i2 = 0; i2 < i3; i2++) {
...
void Element::get_rum_dfd0(vector<DiffDoub0>& rvec, vector<double>& d_rd_a, bool get_matrix, bool actual_props, bool n_lgeom, DiffDoub0StressPrereq& pre, vector<Node>& nd_ar, vector<DesignVariable>& dv_ar) {
	int i1;
	int i2;
	int i3;
	int i4;
	...
	if (dof_per_nd == 6) {
		if (n_lgeom) {
		    get_inst_ori_dfd0(pre.inst_ori, pre.loc_ori, pre.glob_disp, 1);
		}
		if (!actual_props) {
			for (i1 = 0; i1 < 36; i1++) {
	...
	for (i1 = 0; i1 < num_ip; i1++) {
		...
		get_ip_data_dfd0(n_vec, d_ndx, det_j, pre.loc_nds, tmp_s);
		...
		if (dof_per_nd == 6) {
			...
			for (i2 = 0; i2 < nd_dof; i2++) {
				nd1 = dof_table[i7];
				dof1 = dof_table[i7 + 1];
				get_inst_disp_dfd0(inst_disp, pre.glob_disp, pre.inst_ori, pre.loc_ori, pre.glob_nds, n_lgeom, i2, max_int);
				i6 = 0;
				i5 = i2;
				for (i3 = 0; i3 < 6; i3++) {
			...
			ar_to_vec_dfd0(tmp61, pre.scr_vec4, 0, 6);
			mat_mul_dfd0(pre.scr_vec5, pre.mmat, pre.scr_vec4, 6, 6, 1);
			mat_mul_dfd0(pre.scr_vec4, pre.scr_vec5, pre.bmat, 1, 6, nd_dof);
			// update rvec
			for (i2 = 0; i2 < nd_dof; i2++) {
				tmp.set_val_dfd0(pre.scr_vec4[i2]);
				tmp.mult(det_jwt);
				rvec[i2].add(tmp);
			}
			if (get_matrix) {
				mat_mul_dfd0(pre.cbmat, pre.mmat, pre.bmat, 6, 6, nd_dof);
				i4 = 6 * nd_dof;
				for (i2 = 0; i2 < i4; i2++) {
			...
			ar_to_vec_dfd0(n_vec, pre.scr_vec4, 0, 11);
			mat_mul_dfd0(pre.bmat, pre.glob_acc, pre.scr_vec4, 3, num_nds, 1);
			for (i2 = 0; i2 < nd_dof; i2++) {
				nd1 = dof_table[2 * i2];
...
void Element::get_rud_dfd0(vector<DiffDoub0>& rvec, vector<double>& d_rd_v, bool get_matrix, JobCommand& cmd, DiffDoub0StressPrereq& pre, vector<Node>& nd_ar, vector<DesignVariable>& dv_ar) {
	int i1;
	int i2;
	int i3;
	int i4;
	...
	if (cmd.ray_damp_cm > 0.0) {
		...
		get_rum_dfd0(rtmp, d_rtmp, get_matrix, true, cmd.nonlinear_geom, pre, nd_ar, dv_ar);
		for (i1 = 0; i1 < nd_dof; i1++) {
			rvec[i1].add(rtmp[i1]);
		}
		if (get_matrix) {
	...
	if (cmd.ray_damp_ck > 0.0) {
		...
		get_ruk_dfd0(rtmp, d_rtmp, d_rd_t, get_matrix, cmd.nonlinear_geom, pre, nd_ar, dv_ar);
		for (i1 = 0; i1 < nd_dof; i1++) {
			rvec[i1].add(rtmp[i1]);
		}
		if (get_matrix) {
	...
	if (d_non_zero) {
		...
		for (i1 = 0; i1 < num_ip; i1++) {
			...
			wt_det_j.mult(det_j);
			if (dof_per_nd == 3) {
				ar_to_vec_dfd0(d_ndx, pre.scr_vec4, 0, 33);
				mat_mul_dfd0(pre.scr_vec5, pre.glob_disp, pre.scr_vec4, 3, n_dim, 3);
				vec_to_ar_dfd0(ux, pre.scr_vec5, 0, 9);
				for (i2 = 0; i2 < nd_dof; i2++) {
					get_solid_strain_dfd0(strain, ux, d_ndx, pre.loc_ori, i2, max_int, cmd.nonlinear_geom);
					i4 = i2;
					for (i3 = 0; i3 < 6; i3++) {
						//i4 = i3 * nd_dof + i2;
						pre.bmat[i4].set_val_dfd0(strain[i3]);
				...
				for (i2 = 0; i2 < nd_dof; i2++) {
					get_section_def_dfd0(sec_def, pre.glob_disp, pre.inst_ori, pre.loc_ori, pre.glob_nds, d_ndx, n_vec, cmd.nonlinear_geom, i2, max_int);
					i4 = i2;
					for (i3 = 0; i3 < 6; i3++) {
						pre.bmat[i4].set_val_dfd0(sec_def[i3]);
						i4 += nd_dof;
			...
			ar_to_vec_dfd0(bvel, pre.scr_vec4, 0, 9);
			mat_mul_dfd0(pre.scr_vec5, pre.dmat, pre.scr_vec4, def_dim, def_dim, 1);
			mat_mul_dfd0(pre.scr_vec4, pre.scr_vec5, pre.bmat, 1, def_dim, nd_dof);
			for (i2 = 0; i2 < nd_dof; i2++) {
				pre.scr_vec4[i2].mult(wt_det_j);
				rvec[i2].add(pre.scr_vec4[i2]);
			}
			if (get_matrix) {
				mat_mul_dfd0(pre.cbmat, pre.dmat, pre.bmat, def_dim, def_dim, nd_dof);
				i3 = nd_dof * def_dim;
				for (i2 = 0; i2 < i3; i2++) {
...
void Element::get_ru_dfd0(vector<DiffDoub0>& glob_r, SparseMat& globd_rdu, bool get_matrix, JobCommand& cmd, DiffDoub0StressPrereq& pre, vector<Node>& nd_ar, vector<DesignVariable>& dv_ar) {
	int i1;
	int i2;
	int i3;
	int i4;
	...
	if (type == 21) {
		get_ru_frc_fld_dfd0(glob_r, globd_rdu, get_matrix, cmd, pre, nd_ar);
		return;
	}

	if (type != 1) {
		get_ruk_dfd0(rvec, d_rdu, d_rd_t, get_matrix, cmd.nonlinear_geom, pre, nd_ar, dv_ar);
	}

	if (num_int_dof > 0) {
		i2 = nd_dof;
		for (i1 = 0; i1 < num_int_dof; i1++) {
			internal_ru[i1].set_val_dfd0(rvec[i2]);
			i2++;
		}
		if (get_matrix) {
			i4 = 0;
	...
	if(cmd.dynamic) {
		if (get_matrix) {
			if (cmd.lump_mass) {
				...
				get_rum_dfd0(rtmp, d_rtmp, false, true, cmd.nonlinear_geom, pre, nd_ar, dv_ar);
				c1 = cmd.time_step;
				...
				get_rum_dfd0(rtmp, d_rtmp, true, true, cmd.nonlinear_geom, pre, nd_ar, dv_ar);
				for (i1 = 0; i1 < nd_dof; i1++) {
					rvec[i1].add(rtmp[i1]);
				}
				c1 = cmd.time_step;
			...
			if (type != 1) {
				get_rud_dfd0(rtmp, d_rtmp, true, cmd, pre, nd_ar, dv_ar);
				for (i1 = 0; i1 < nd_dof; i1++) {
					rvec[i1].add(rtmp[i1]);
				}
			...
			if (type != 1) {
				get_rud_dfd0(rtmp, d_rtmp, false, cmd, pre, nd_ar, dv_ar);
				for (i1 = 0; i1 < nd_dof; i1++) {
					rvec[i1].add(rtmp[i1]);
				}
			}
...
void Element::get_rtk_dfd0(vector<DiffDoub0>& rvec, vector<double>& d_rd_t, bool get_matrix, DiffDoub0StressPrereq& pre) {
	int i1;
	int i2;
	int i3;
	DiffDoub0 n_vec[11];
	...
	for (i1 = 0; i1 < num_ip; i1++) {
		...
		tmp.set_val(ip_wt[i1]);
		det_j.mult(tmp);
		vec_to_ar_dfd0(tmp_t, pre.glob_temp, 0, 10);
		mat_mul_ar_dfd0(grad_t, tmp_t, d_ndx, 1, num_nds, 3);
		vec_to_ar_dfd0(tmp_tc, pre.tcmat, 0, 9);
		mat_mul_ar_dfd0(q_vec, tmp_tc, grad_t, 3, 3, 1);
		mat_mul_ar_dfd0(rtmp, d_ndx, q_vec, num_nds, 3, 1);
		for (i2 = 0; i2 < num_nds; i2++) {
			rtmp[i2].mult(det_j);
			rvec[i2].add(rtmp[i2]);
		}
		if (get_matrix) {
			transpose_ar_dfd0(d_nt, d_ndx, num_nds,3);
			mat_mul_ar_dfd0(tmp_ar, tmp_tc, d_nt, 3, 3, num_nds);
			mat_mul_ar_dfd0(d_rtmp, d_ndx, tmp_ar, num_nds, 3, num_nds);
			i3 = num_nds * num_nds;
			for (i2 = 0; i2 < i3; i2++) {
...
void Element::get_rtm_dfd0(vector<DiffDoub0>& rvec, vector<double>& d_rd_tdot, bool get_matrix, bool actual_props, DiffDoub0StressPrereq& pre) {
	int i1;
	int i2;
	int i3;
	DiffDoub0 n_vec[11];
	...
	for (i1 = 0; i1 < num_ip; i1++) {
		...
		if (get_matrix) {
			mat_mul_ar_dfd0(d_rtmp,n_vec,n_vec,num_nds,1,num_nds);
			i3 = num_nds * num_nds;
			for (i2 = 0; i2 < i3; i2++) {
...
void Element::get_rt_dfd0(vector<DiffDoub0>& glob_r, SparseMat& globd_rd_t, bool get_matrix, JobCommand& cmd, DiffDoub0StressPrereq& pre, vector<Node>& nd_ar) {
	int i1;
	int i2;
	int i3;
	int glob_ind1;
	...
	if (type == 21) {
		get_rt_frc_fld_dfd0(glob_r, globd_rd_t, pre.scr_mat1, pre.scr_mat2, get_matrix, cmd, pre, nd_ar);
		return;
	}

	if (type != 1) {
		get_rtk_dfd0(rvec, d_rd_t, get_matrix, pre);
	}

	if (cmd.dynamic) {
		get_rtm_dfd0(rtmp, d_rtmp, get_matrix, true, pre);
		i3 = 0;
		for (i1 = 0; i1 < num_nds; i1++) {
			rvec[i1].add(rtmp[i1]);
			if (get_matrix) {
...
void Element::get_ru_frc_fld_dfd0(vector<DiffDoub0>& glob_r, SparseMat& globd_rdu, bool get_matrix, JobCommand& cmd, DiffDoub0StressPrereq& pre, vector<Node>& nd_ar) {
	int i1;
	int i2;
	int i3;
	int i4;
	...
	mat_mul_ar_dfd0(d_distd_u, d_vec, d_dvecd_u, 1, 3, 6);
	tmp.set_val(1.0);
	tmp.dvd(dist);
	for (i1 = 0; i1 < 6; i1++) {
	...
	mat_mul_ar_dfd0(df_n1d_u, d_vec, d_distd_u, 3, 1, 6);
	tmp.set_val(1.0);
...
void Element::get_rt_frc_fld_dfd0(vector<DiffDoub0>& glob_r, SparseMat& globd_rd_t, vector<double>& d_rd_u, vector<double>& d_rd_v, bool get_matrix, JobCommand& cmd, DiffDoub0StressPrereq& pre, vector<Node>& nd_ar) {
	int i1;
	int i2;
	int i3;
	int i4;
	...
	if (get_matrix) {
		...
		mat_mul_ar_dfd0(tmp_mat, dv_vec, d_distd_u, 3, 1, 6);
		tmp.set_val_dfd0(pre.frc_fld_coef[1]);
		...
		mat_mul_ar_dfd0(tmp_mat2, tmp_mat, d_distd_u, 2, 1, 6);
		...
		// work dissipation term
		mat_mul_ar_dfd0(tmp_mat, dv_vec, df_n1d_u, 1, 3, 6);
		i3 = 0;
		for (i1 = 0; i1 < 2; i1++) {
			for (i2 = 0; i2 < 6; i2++) {
		...
		mat_mul_ar_dfd0(tmp_mat, dv_vec, df_n1d_v, 1, 3, 6);
		i3 = 0;
		for (i1 = 0; i1 < 2; i1++) {
			for (i2 = 0; i2 < 6; i2++) {
		...
		mat_mul_ar_dfd0(tmp_mat, f_n1, d_dvecd_u, 1, 3, 6);
		i3 = 0;
		for (i1 = 0; i1 < 2; i1++) {
			for (i2 = 0; i2 < 6; i2++) {
				d_rd_v[i3] -= 0.5 * tmp_mat[i2].val;
...
void Element::get_app_load_dfd0(vector<DiffDoub0>& app_ld, Load& ld_pt, bool n_lgeom, DiffDoub0StressPrereq& pre, vector<Section>& sec_ar, vector<Face>& fc_ar, vector<Node>& nd_ar, vector<DesignVariable>& dv_ar) {
	int i1;
	int i2;
	int i3;
	int i4;
	...
	if (ld_type == "bodyForce") {
		...
		get_rum_dfd0(el_app_ld, d_rd_a, false, false, n_lgeom, pre, nd_ar, dv_ar);
		i2 = 0;
		for (i1 = 0; i1 < nd_dof; i1++) {
			dof = dof_table[i2 + 1];
		...
		if (type == 41 || type == 3) {
			...
			for (i1 = 0; i1 < num_lay; i1++) {
				get_volume_dfd0(tmp, pre, i1, sec_ar, dv_ar);
				el_vol.add(tmp);
			}
		}
		else {
			get_volume_dfd0(el_vol, pre, 0, sec_ar, dv_ar);
		}
		...
		get_rum_dfd0(el_app_ld, d_rd_a, false, true, n_lgeom, pre, nd_ar, dv_ar);
	}
		...
		for (auto& fi : faces) {
			...
			if (this_fc.on_surf) {
				this_fc.get_area_normal_dfd0(fc_area, fc_norm, nd_ar, dv_ar);
				dp.set_val(0.0);
				for (i1 = 0; i1 < 3; i1++) {
...
void Element::get_app_therm_load_dfd0(vector<DiffDoub0>& app_ld, Load& ld_pt, DiffDoub0StressPrereq& pre, vector<Section>& sec_ar, vector<Face>& fc_ar, vector<Node>& nd_ar, vector<DesignVariable>& dv_ar) {
	int i1;
	int i2;
	int glob_ind;
	int num_lay = sec_ar[sect_ptr].layers.size();
	...
	if (ld_type == "bodyHeatGen") {
		...
		get_rtm_dfd0(el_app_ld, d_rd_t,false,false,pre);
		tot_hg.set_val(0.0);
		for (i1 = 0; i1 < num_nds; i1++) {
...
void Element::get_ruk_dfd1(vector<DiffDoub1>& rvec, vector<double>& d_rdu, vector<double>& d_rd_t, bool get_matrix, bool n_lgeom, DiffDoub1StressPrereq& pre, vector<Node>& nd_ar, vector<DesignVariable>& dv_ar) {
	int i1;
	int i2;
	int i3;
	int i4;
	...
	vec_to_ar_dfd1(tmp_c, pre.cmat, 0, 81);
	vec_to_ar_dfd1(tmp_gd, pre.glob_disp, 0, 60);
	vec_to_ar_dfd1(tmp_te, pre.therm_exp, 0, 6);
	...
	for (i1 = 0; i1 < num_ip; i1++) {
		get_ip_data_dfd1(n_vec,d_ndx,det_j,pre.loc_nds,&int_pts[3*i1]);
		...
		if(dof_per_nd == 3) {
			mat_mul_ar_dfd1(ux,tmp_gd,d_ndx,3,n_dim,3);
			...
			mat_mul_ar_dfd1(stress,tmp_c,strain,6,6,1);
			if (get_matrix) {
				mat_mul_ar_dfd1(cte, tmp_c, tmp_te, 6, 6, 1);
				mat_mul_ar_dfd1(cten, cte, n_vec, 6, 1, num_nds);
			}
			for (i2 = 0; i2 < tot_dof; i2++) {
				get_solid_strain_dfd1(strain,ux,d_ndx,pre.loc_ori,i2,max_int,n_lgeom);
				i4 = i2;
				for (i3 = 0; i3 < 6; i3++) {
				...
				if(n_lgeom && get_matrix) {
					...
					i6 = i5;
					for (i3 = i2; i3 < tot_dof; i3++) {
						get_solid_strain_dfd1(strain,ux,d_ndx,pre.loc_ori,i2,i3,n_lgeom);
						for (i4 = 0; i4 < 6; i4++) {
							d_rdu[i5]+= stress[i4].val*strain[i4].val*d_jwt.val;
						}
						d_rdu[i6] = d_rdu[i5];
			...
			mat_mul_ar_dfd1(sec_fc_mom,tmp_c,sec_def,def_dim,def_dim,1);
			for (i2 = 0; i2 < 6; i2++) {
			...
			if (get_matrix) {
				mat_mul_ar_dfd1(cten, tmp_te, n_vec, 6, 1, num_nds);
			}
			for (i2 = 0; i2 < tot_dof; i2++) {
				get_section_def_dfd1(sec_def,pre.glob_disp,pre.inst_ori,pre.loc_ori,pre.glob_nds,d_ndx,n_vec,n_lgeom,i2,max_int);
				i4 = i2;
				for (i3 = 0; i3 < def_dim; i3++) {
				...
				if(n_lgeom && get_matrix) {
					...
					i6 = i5;
					for (i3 = i2; i3 < tot_dof; i3++) {
						get_section_def_dfd1(sec_def,pre.glob_disp,pre.inst_ori,pre.loc_ori,pre.glob_nds,d_ndx,n_vec,n_lgeom,i2,i3);
						for (i4 = 0; i4 < def_dim; i4++) {
							d_rdu[i5]+= sec_fc_mom[i4].val*sec_def[i4].val*d_jwt.val;
						}
						d_rdu[i6] = d_rdu[i5];
		...
		if(get_matrix) {
			mat_mul_dfd1(pre.cbmat,pre.cmat,pre.bmat,def_dim,def_dim,tot_dof);
			i3 = def_dim*tot_dof;
			for (i2 = 0; i2 < i3; i2++) {
				pre.cbmat[i2].mult(d_jwt);
			}
...
void Element::get_rum_dfd1(vector<DiffDoub1>& rvec, vector<double>& d_rd_a, bool get_matrix, bool actual_props, bool n_lgeom, DiffDoub1StressPrereq& pre, vector<Node>& nd_ar, vector<DesignVariable>& dv_ar) {
	int i1;
	int i2;
	int i3;
	int i4;
	...
	if (dof_per_nd == 6) {
		if (n_lgeom) {
		    get_inst_ori_dfd1(pre.inst_ori, pre.loc_ori, pre.glob_disp, 1);
		}
		if (!actual_props) {
			for (i1 = 0; i1 < 36; i1++) {
	...
	for (i1 = 0; i1 < num_ip; i1++) {
		i2 = 3 * i1;
		vec_to_ar(tmp_s, int_pts, i2, i2 + 3);
		get_ip_data_dfd1(n_vec, d_ndx, det_j, pre.loc_nds, tmp_s);
		det_jwt.set_val(ip_wt[i1]);
		det_jwt.mult(det_j);
		if (dof_per_nd == 6) {
			// build matrix [d_u^i/d_u^g] * {n}
			...
			for (i2 = 0; i2 < nd_dof; i2++) {
				nd1 = dof_table[i7];
				dof1 = dof_table[i7 + 1];
				get_inst_disp_dfd1(inst_disp, pre.glob_disp, pre.inst_ori, pre.loc_ori, pre.glob_nds, n_lgeom, i2, max_int);
				i6 = 0;
				i5 = i2;
				for (i3 = 0; i3 < 6; i3++) {
					pre.bmat[i5].set_val(0.0);
			...
			// pre.scr_vec5 = [m][b]{a}, originally tmp62
			ar_to_vec_dfd1(tmp61, pre.scr_vec4, 0, 6);
			mat_mul_dfd1(pre.scr_vec5, pre.mmat, pre.scr_vec4, 6, 6, 1);
			mat_mul_dfd1(pre.scr_vec4, pre.scr_vec5, pre.bmat, 1, 6, nd_dof);
			// update rvec
			for (i2 = 0; i2 < nd_dof; i2++) {
				tmp.set_val_dfd1(pre.scr_vec4[i2]);
				tmp.mult(det_jwt);
				rvec[i2].add(tmp);
			}
			if (get_matrix) {
				mat_mul_dfd1(pre.cbmat, pre.mmat, pre.bmat, 6, 6, nd_dof);
				i4 = 6 * nd_dof;
				for (i2 = 0; i2 < i4; i2++) {
					pre.cbmat[i2].mult(det_jwt);
				}
			...
			ar_to_vec_dfd1(n_vec, pre.scr_vec4, 0, 11);
			mat_mul_dfd1(pre.bmat, pre.glob_acc, pre.scr_vec4, 3, num_nds, 1);
			for (i2 = 0; i2 < nd_dof; i2++) {
				nd1 = dof_table[2 * i2];
				dof1 = dof_table[2 * i2 + 1];
...
void Element::get_rud_dfd1(vector<DiffDoub1>& rvec, vector<double>& d_rd_v, bool get_matrix, JobCommand& cmd, DiffDoub1StressPrereq& pre, vector<Node>& nd_ar, vector<DesignVariable>& dv_ar) {
	int i1;
	int i2;
	int i3;
	int i4;
	...
	if (cmd.ray_damp_cm > 0.0) {
		...
		get_rum_dfd1(rtmp, d_rtmp, get_matrix, true, cmd.nonlinear_geom, pre, nd_ar, dv_ar);
		for (i1 = 0; i1 < nd_dof; i1++) {
			rvec[i1].add(rtmp[i1]);
		}
		if (get_matrix) {
	...
	if (cmd.ray_damp_ck > 0.0) {
		...
		get_ruk_dfd1(rtmp, d_rtmp, d_rd_t, get_matrix, cmd.nonlinear_geom, pre, nd_ar, dv_ar);
		for (i1 = 0; i1 < nd_dof; i1++) {
			rvec[i1].add(rtmp[i1]);
		}
		if (get_matrix) {
	...
	if (d_non_zero) {
		...
		for (i1 = 0; i1 < num_ip; i1++) {
			...
			wt_det_j.mult(det_j);
			if (dof_per_nd == 3) {
				ar_to_vec_dfd1(d_ndx, pre.scr_vec4, 0, 33);
				mat_mul_dfd1(pre.scr_vec5, pre.glob_disp, pre.scr_vec4, 3, n_dim, 3);
				vec_to_ar_dfd1(ux, pre.scr_vec5, 0, 9);
				for (i2 = 0; i2 < nd_dof; i2++) {
					get_solid_strain_dfd1(strain, ux, d_ndx, pre.loc_ori, i2, max_int, cmd.nonlinear_geom);
					i4 = i2;
					for (i3 = 0; i3 < 6; i3++) {
						//i4 = i3 * nd_dof + i2;
						pre.bmat[i4].set_val_dfd1(strain[i3]);
				...
				for (i2 = 0; i2 < nd_dof; i2++) {
					get_section_def_dfd1(sec_def, pre.glob_disp, pre.inst_ori, pre.loc_ori, pre.glob_nds, d_ndx, n_vec, cmd.nonlinear_geom, i2, max_int);
					i4 = i2;
					for (i3 = 0; i3 < 6; i3++) {
						pre.bmat[i4].set_val_dfd1(sec_def[i3]);
						i4 += nd_dof;
			...
			ar_to_vec_dfd1(bvel, pre.scr_vec4, 0, 9);
			mat_mul_dfd1(pre.scr_vec5, pre.dmat, pre.scr_vec4, def_dim, def_dim, 1);
			mat_mul_dfd1(pre.scr_vec4, pre.scr_vec5, pre.bmat, 1, def_dim, nd_dof);
			for (i2 = 0; i2 < nd_dof; i2++) {
				pre.scr_vec4[i2].mult(wt_det_j);
				rvec[i2].add(pre.scr_vec4[i2]);
			}
			if (get_matrix) {
				mat_mul_dfd1(pre.cbmat, pre.dmat, pre.bmat, def_dim, def_dim, nd_dof);
				i3 = nd_dof * def_dim;
				for (i2 = 0; i2 < i3; i2++) {
					pre.cbmat[i2].mult(wt_det_j);
				}
...
void Element::get_ru_dfd1(vector<DiffDoub1>& glob_r, SparseMat& globd_rdu, bool get_matrix, JobCommand& cmd, DiffDoub1StressPrereq& pre, vector<Node>& nd_ar, vector<DesignVariable>& dv_ar) {
	int i1;
	int i2;
	int i3;
	int i4;
	...
	if (type == 21) {
		get_ru_frc_fld_dfd1(glob_r, globd_rdu, get_matrix, cmd, pre, nd_ar);
		return;
	}

	if (type != 1) {
		get_ruk_dfd1(rvec, d_rdu, d_rd_t, get_matrix, cmd.nonlinear_geom, pre, nd_ar, dv_ar);
	}

	if (num_int_dof > 0) {
		i2 = nd_dof;
		for (i1 = 0; i1 < num_int_dof; i1++) {
			internal_ru[i1].set_val_dfd1(rvec[i2]);
			i2++;
		}
		if (get_matrix) {
			i4 = 0;
	...
	if(cmd.dynamic) {
		if (get_matrix) {
			if (cmd.lump_mass) {
				...
				get_rum_dfd1(rtmp, d_rtmp, false, true, cmd.nonlinear_geom, pre, nd_ar, dv_ar);
				c1 = cmd.time_step;
				c1 = -1.0 / (c1 * c1 * (cmd.newmark_beta - cmd.newmark_gamma));
				i2 = 0;
				i3 = tot_dof + 1;
				...
				get_rum_dfd1(rtmp, d_rtmp, true, true, cmd.nonlinear_geom, pre, nd_ar, dv_ar);
				for (i1 = 0; i1 < nd_dof; i1++) {
					rvec[i1].add(rtmp[i1]);
				}
				c1 = cmd.time_step;
			...
			if (type != 1) {
				get_rud_dfd1(rtmp, d_rtmp, true, cmd, pre, nd_ar, dv_ar);
				for (i1 = 0; i1 < nd_dof; i1++) {
					rvec[i1].add(rtmp[i1]);
				}
				c2 = cmd.time_step * cmd.newmark_gamma * c1;
			...
			if (type != 1) {
				get_rud_dfd1(rtmp, d_rtmp, false, cmd, pre, nd_ar, dv_ar);
				for (i1 = 0; i1 < nd_dof; i1++) {
					rvec[i1].add(rtmp[i1]);
				}
			}
...
void Element::get_rtk_dfd1(vector<DiffDoub1>& rvec, vector<double>& d_rd_t, bool get_matrix, DiffDoub1StressPrereq& pre) {
	int i1;
	int i2;
	int i3;
	DiffDoub1 n_vec[11];
	...
	for (i1 = 0; i1 < num_ip; i1++) {
		...
		tmp.set_val(ip_wt[i1]);
		det_j.mult(tmp);
		vec_to_ar_dfd1(tmp_t, pre.glob_temp, 0, 10);
		mat_mul_ar_dfd1(grad_t, tmp_t, d_ndx, 1, num_nds, 3);
		vec_to_ar_dfd1(tmp_tc, pre.tcmat, 0, 9);
		mat_mul_ar_dfd1(q_vec, tmp_tc, grad_t, 3, 3, 1);
		mat_mul_ar_dfd1(rtmp, d_ndx, q_vec, num_nds, 3, 1);
		for (i2 = 0; i2 < num_nds; i2++) {
			rtmp[i2].mult(det_j);
			rvec[i2].add(rtmp[i2]);
		}
		if (get_matrix) {
			transpose_ar_dfd1(d_nt, d_ndx, num_nds,3);
			mat_mul_ar_dfd1(tmp_ar, tmp_tc, d_nt, 3, 3, num_nds);
			mat_mul_ar_dfd1(d_rtmp, d_ndx, tmp_ar, num_nds, 3, num_nds);
			i3 = num_nds * num_nds;
			for (i2 = 0; i2 < i3; i2++) {
				d_rd_t[i2] += d_rtmp[i2].val*det_j.val;
			}
...
void Element::get_rtm_dfd1(vector<DiffDoub1>& rvec, vector<double>& d_rd_tdot, bool get_matrix, bool actual_props, DiffDoub1StressPrereq& pre) {
	int i1;
	int i2;
	int i3;
	DiffDoub1 n_vec[11];
	...
	for (i1 = 0; i1 < num_ip; i1++) {
		...
		if (get_matrix) {
			mat_mul_ar_dfd1(d_rtmp,n_vec,n_vec,num_nds,1,num_nds);
			i3 = num_nds * num_nds;
			for (i2 = 0; i2 < i3; i2++) {
				d_rd_tdot[i2] += d_rtmp[i2].val * pre.spec_heat.val * det_j.val;
			}
...
void Element::get_rt_dfd1(vector<DiffDoub1>& glob_r, SparseMat& globd_rd_t, bool get_matrix, JobCommand& cmd, DiffDoub1StressPrereq& pre, vector<Node>& nd_ar) {
	int i1;
	int i2;
	int i3;
	int glob_ind1;
	...
	if (type == 21) {
		get_rt_frc_fld_dfd1(glob_r, globd_rd_t, pre.scr_mat1, pre.scr_mat2, get_matrix, cmd, pre, nd_ar);
		return;
	}

	if (type != 1) {
		get_rtk_dfd1(rvec, d_rd_t, get_matrix, pre);
	}

	if (cmd.dynamic) {
		get_rtm_dfd1(rtmp, d_rtmp, get_matrix, true, pre);
		i3 = 0;
		for (i1 = 0; i1 < num_nds; i1++) {
			rvec[i1].add(rtmp[i1]);
			if (get_matrix) {
...
void Element::get_ru_frc_fld_dfd1(vector<DiffDoub1>& glob_r, SparseMat& globd_rdu, bool get_matrix, JobCommand& cmd, DiffDoub1StressPrereq& pre, vector<Node>& nd_ar) {
	int i1;
	int i2;
	int i3;
	int i4;
	...
	mat_mul_ar_dfd1(d_distd_u, d_vec, d_dvecd_u, 1, 3, 6);
	tmp.set_val(1.0);
	tmp.dvd(dist);
	for (i1 = 0; i1 < 6; i1++) {
		d_distd_u[i1].mult(tmp);
	...
	mat_mul_ar_dfd1(df_n1d_u, d_vec, d_distd_u, 3, 1, 6);
	tmp.set_val(1.0);
	tmp.add(pre.frc_fld_exp[0]);
	tmp.neg();
	tmp.mult(pre.frc_fld_coef[0]);
...
void Element::get_rt_frc_fld_dfd1(vector<DiffDoub1>& glob_r, SparseMat& globd_rd_t, vector<double>& d_rd_u, vector<double>& d_rd_v, bool get_matrix, JobCommand& cmd, DiffDoub1StressPrereq& pre, vector<Node>& nd_ar) {
	int i1;
	int i2;
	int i3;
	int i4;
	...
	if (get_matrix) {
		...
		mat_mul_ar_dfd1(tmp_mat, dv_vec, d_distd_u, 3, 1, 6);
		tmp.set_val_dfd1(pre.frc_fld_coef[1]);
		tmp.mult(pre.frc_fld_exp[1]);
		tmp.dvd(dto_p1);
		tmp.neg();
		...
		mat_mul_ar_dfd1(tmp_mat2, tmp_mat, d_distd_u, 2, 1, 6);
		...
		// work dissipation term
		mat_mul_ar_dfd1(tmp_mat, dv_vec, df_n1d_u, 1, 3, 6);
		i3 = 0;
		for (i1 = 0; i1 < 2; i1++) {
			for (i2 = 0; i2 < 6; i2++) {
				d_rd_u[i3] -= 0.5 * tmp_mat[i2].val;
		...
		mat_mul_ar_dfd1(tmp_mat, dv_vec, df_n1d_v, 1, 3, 6);
		i3 = 0;
		for (i1 = 0; i1 < 2; i1++) {
			for (i2 = 0; i2 < 6; i2++) {
				d_rd_v[i3] -= 0.5 * tmp_mat[i2].val;
		...
		mat_mul_ar_dfd1(tmp_mat, f_n1, d_dvecd_u, 1, 3, 6);
		i3 = 0;
		for (i1 = 0; i1 < 2; i1++) {
			for (i2 = 0; i2 < 6; i2++) {
				d_rd_v[i3] -= 0.5 * tmp_mat[i2].val;
...
void Element::get_app_load_dfd1(vector<DiffDoub1>& app_ld, Load& ld_pt, bool n_lgeom, DiffDoub1StressPrereq& pre, vector<Section>& sec_ar, vector<Face>& fc_ar, vector<Node>& nd_ar, vector<DesignVariable>& dv_ar) {
	int i1;
	int i2;
	int i3;
	int i4;
	...
	if (ld_type == "bodyForce") {
		...
		get_rum_dfd1(el_app_ld, d_rd_a, false, false, n_lgeom, pre, nd_ar, dv_ar);
		i2 = 0;
		for (i1 = 0; i1 < nd_dof; i1++) {
			dof = dof_table[i2 + 1];
			tot_nd_f[dof].add(el_app_ld[i1]);
		...
		if (type == 41 || type == 3) {
			el_vol.set_val(0.0);
			for (i1 = 0; i1 < num_lay; i1++) {
				get_volume_dfd1(tmp, pre, i1, sec_ar, dv_ar);
				el_vol.add(tmp);
			}
		}
		else {
			get_volume_dfd1(el_vol, pre, 0, sec_ar, dv_ar);
		}
		...
		get_rum_dfd1(el_app_ld, d_rd_a, false, true, n_lgeom, pre, nd_ar, dv_ar);
	}
	else if (ld_type == "centrifugal") {
		ang_vel2.set_val(ld_pt.angular_vel);
		ang_vel2.sqr();
		...
		for (auto& fi : faces) {
			Face& this_fc = fc_ar[fi];
			if (this_fc.on_surf) {
				this_fc.get_area_normal_dfd1(fc_area, fc_norm, nd_ar, dv_ar);
				dp.set_val(0.0);
				for (i1 = 0; i1 < 3; i1++) {
					tmp.set_val(ld_pt.normal_dir[i1]);
					tmp.mult(fc_norm[i1]);
...
void Element::get_app_therm_load_dfd1(vector<DiffDoub1>& app_ld, Load& ld_pt, DiffDoub1StressPrereq& pre, vector<Section>& sec_ar, vector<Face>& fc_ar, vector<Node>& nd_ar, vector<DesignVariable>& dv_ar) {
	int i1;
	int i2;
	int glob_ind;
	int num_lay = sec_ar[sect_ptr].layers.size();
	...
	if (ld_type == "bodyHeatGen") {
		...
		get_rtm_dfd1(el_app_ld, d_rd_t,false,false,pre);
		tot_hg.set_val(0.0);
		for (i1 = 0; i1 < num_nds; i1++) {
			tot_hg.add(el_app_ld[i1]);
		}
