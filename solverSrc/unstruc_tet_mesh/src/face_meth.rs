...
impl MeshFace {
    ...
    pub fn init_norm_dir(&mut self, nd_ar : &Vec<MeshNode>) {
        ...
        let mut mag : f64;
        let mut area : f64;
        let mut n1 = &nd_ar[self.nodes[0]].coord;
        let mut n2 = &nd_ar[self.nodes[1]].coord;
        let mut n3 = &nd_ar[self.nodes[2]].coord;
        let mut i1 : usize;
        for i1 in 0..3 {
            v1[i1] = n2[i1] - n1[i1];
            v2[i1] = n3[i1] - n1[i1];
    ...
    pub fn norm_dir_from_el_cent(&mut self, cent : &[f64], nd_ar : &Vec<MeshNode>) {
        ...
        let mut dp : f64 =  d_vec[0] * self.norm_dir[0] + d_vec[1] * self.norm_dir[1] + d_vec[2] * self.norm_dir[2];
        if (dp < 0.0) {
            self.norm_dir[0]  *=  -1.0;
            self.norm_dir[1]  *=  -1.0;
            self.norm_dir[2]  *=  -1.0;
    ...
    pub fn get_intersection(& self, out_param : &mut [f64], pt : &[f64], vec : & [f64], nd_ar : & Vec<MeshNode>) -> bool {
        ...
        let mut det : f64 =  mat[0] * mat[4] * mat[8];
    ...
    pub fn edges_intersect(& self, fc : usize, dist_tol : f64, nd_ar : &Vec<MeshNode>, fc_ar : &Vec<MeshFace>) -> bool {
        ...
        let mut fc_nodes = &fc_ar[fc].nodes;
        
        for i1 in 0..3 {
            let mut n11 = &nd_ar[self.nodes[i1]].coord;
            i3 = i1 + 1;
            if (i3 > 2) {
                i3 = 0;
            }
            let mut n21 = &nd_ar[self.nodes[i3]].coord;
            for i2 in 0..3 {
                let mut n12 = &nd_ar[fc_nodes[i2]].coord;
                i4 = i2 + 1;
                if (i4 > 2) {
                    i4 = 0;
                }
                let mut n22 = &nd_ar[fc_nodes[i4]].coord;
                v1[0] = n21[0] - n11[0];
                v1[1] = n21[1] - n11[1];
                v1[2] = n21[2] - n11[2];
                v2[0] = n22[0] - n12[0];
    ...
    pub fn get_shared_nodes(& self, nd_pts : &mut [usize], shared : &mut [bool], fc : usize, nd_ar : &mut Vec<MeshNode>, fc_ar : & Vec<MeshFace>) -> usize {
        let mut i1 : usize;
        let mut i2 : usize;
        let mut fc_nds = &fc_ar[fc].nodes;
    ...
    pub fn print_info(&mut self, nd_ar : &mut Vec<MeshNode>) {
        ...
        println!("{}", "nodes:" );
        for i1 in 0..3 {
            let mut crd = &nd_ar[self.nodes[i1]].coord;
