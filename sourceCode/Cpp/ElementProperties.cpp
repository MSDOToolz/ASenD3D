...
void Element::getThickOffset(Doub& thickness, Doub& zOffset, DVPt dvAr[]) {
	Doub layerThk;
	Doub dvVal;
	Doub coef;
	...
	int layNum = 0;
	thickness.setVal(0.0);
	while(thisLay) {
		layerThk.setVal(thisLay->getThickness());
		thisDV = designVars.getFirst();
		thisCoef = dvCoef.getFirst();
		while(thisDV) {
			thisDVpt = dvAr[thisDV->value].ptr;
			if(thisDVpt->getCategory() == "thickness" && thisDVpt->getLayer() == layNum) {
	...
	if(dofPerNd == 3) { // solid elements
		...
		if(type == 2) { // beams
			...
			while(thisLay) {
				...
				thknss.setVal(thisLay->getThickness());
